...
export function buildSystemPrompt(
  ...
  const fileRules = includeFileTools
    ? [
        ...
        '- **交付格式**:给用户看的成果(文档/表格/演示)默认用 Office 格式保存——.docx/.xlsx/.pptx;.md 只用于 agent 间的交接记录和规则文件,不作为交给用户的成果格式;用户另有指定时听用户的。',
        '- PPT 能力边界:只能根据用户提供的内容新建或覆盖演示文稿，不能读取或局部修改已有 PPT，不得声称已经看过旧文件；用户要改旧 PPT 时，先索取源文字或大纲并另存新文件。',
      ]
    : []
  return [
